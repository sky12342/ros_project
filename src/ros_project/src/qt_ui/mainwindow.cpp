    ...
    , rosmap_(FunList::getInstance())
{
    ...
    // 初始化 ROS 节点
        ...
        ros_manager_->init("/getmessage", 1024);
        auto ros_worker = ros_manager_->getNode();
        // 创建 SignalSlotProxy 实例，负责管理信号和槽的连接
        signal_slot_proxy_ = std::make_unique<SignalSlotProxy>(ros_worker);
        // 连接信号和槽，通过代理类来管理
        ...
        QObject::connect(this, &MainWindow::signal_Destination_site, signal_slot_proxy_.get(), &SignalSlotProxy::setDestinationSite);
...
{    // 设置 ROS 环境变量
    ...
    // 启动第一个进程 (roslaunch wpr_simulation wpb_stage_robocup.launch)
    process1 = new QProcess(this);
    process1->setProcessEnvironment(env);  // 设置环境变量
    QStringList arguments1 = QStringList() << "wpr_simulation" << "wpb_stage_robocup.launch";
    process1->start("roslaunch", arguments1);
    // 在 Qt 窗口中显示消息
...
void MainWindow::on_navigateion_to_point_clicked()
{
    ...
    ros_manager_->run();  // 在新线程中运行 ros_manager_
}));
}
