 ...
 * 构造函数 初始化日志类
 */
MyLog::MyLog(const string& log_path,const string& log_file_name):log_path_(log_path),log_file_name_(log_file_name)
{   
...
//    FLAGS_MAX_log_days = //旧版本还不支持
    ...
    const char* log_argv = {" "};
    google::InitGoogleLogging(log_argv);//初始化日志系统false
}
